...
To add load to the service while running profiling we can run these command.
	...
	$ hey -m POST -c 100 -n 10000 "http://localhost:5001/search?term=trump&cnn=on&bbc=on&nyt=on"
...
Look at the basic profiling stats from the new endpoint:
	...
	http://localhost:5001/debug/pprof

Capture heap profile:

	http://localhost:5001/debug/pprof/heap

Capture cpu profile:

	http://localhost:5001/debug/pprof/profile

#### Interactive Profiling

Run the Go pprof tool in another window or tab to review alloc space heap information.

	$ go tool pprof http://localhost:5001/debug/pprof/allocs
